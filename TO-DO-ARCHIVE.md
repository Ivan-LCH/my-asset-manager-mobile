...
## 2026-09-03 — 현금흐름 세금 계산 정밀화

### 보유세(재산세+종부세) 현금흐름 반영 (2026-09-02)
> 매탄주공 입주(29년 말) 이후 부과되는 보유세를 현금흐름 세금(연)에 반영.

- [C] **HT-1. RetirementPlan 타입에 holdingTaxAnnual / holdingTaxStartYear 추가**
- [C] **HT-2. buildCashFlow 세금(연)에 개시 연도부터 합산** — 기본 410만원 / 2030년~
- [C] **HT-3. 은퇴 페이지에 보유세 입력 UI** (금액 + 부과 개시 연도)
- [C] **HT-4. 검증 (tsc + 72테스트) + E2E 확인 + push** — 2029년 —, 2030년부터 세금(연) -410만원 반영 확인

### 세금 과세 오류·간소화 교정 (2026-09-03)
> "세금이 너무 많이 나오는 것 같다"는 검토 요청 → 3건 교정.

- [C] **TX-1. 금융소득종합과세 이중과세 수정** — 2천만 초과 배당의 누진 종합세에서 원천징수 15.4% 기납부세액 공제 (실데이터 2045년 세금(연) -2,786만 → -2,193만)
- [C] **TX-2. 배당가산·배당세액공제 반영** — 종합과세 배당 × 10% 가산(법인세액상당) 후 가산액 × 13% 세액공제. TaxBreakdown에 dividendGrossUp/dividendCredit 추가
- [C] **TX-3. 국민연금 연금소득세 교정** — 연금소득공제 1,200만은 사적연금에만 적용, 국민연금(공적)은 공제 없이 합산 누진 (pensionTaxCombined 헬퍼, 적용 2곳)
- [C] **TX-4. 검증 (tsc + 75테스트) + 실데이터 E2E + push** — 테스트 72→75개, 실데이터 백업 import로 연도별 세금(연) 확인

---

## 2026-08-30 — 주식 등록·이력 입력 간소화
> 시세/검색 자동화로 수동 입력 최소화.

- [C] **AT-1. 이력 입력 시 단가 자동 조회** — HistoryTable 추가 폼에서 티커 있으면 Yahoo 시세 자동 채움(수동 수정 가능), 티커 없으면 수동 유지
- [C] **AT-2. 주식 등록 시 종목 검색 자동완성** — 자산명 필드에 StockSearch 연동, 선택 시 티커·통화·현재가 자동 채움
- [C] **AT-3. 주요 주식 티커 프리셋** — 국내/미국 주식·ETF 40종 (한글 별칭 매칭, API 결과와 병합·중복 제거)
- [C] **AT-4. 배포용 /api/search 서버리스 함수** — 공통 로직 api/_search.ts 추출 + Vercel api/search.ts (.js 확장자 import로 ESM 해석 수정)
- [C] **AT-5. 검증 (tsc + 72테스트) + E2E 확인 + push** — 한글 '나스닥'→프리셋 매칭·시세 26,235 자동입력, 이력 추가 시 현재가 18,190 자동입력 확인
...
## 2026-08-30 — 간소화·시세 이력 정비 (사용자 확인완료 이관)
> TO-DO-LIST.md의 CF/TILE/ACC 섹션과 이후 진행된 간소화·시세 백필 작업 일괄 이관.

### 현금흐름·연금 모델 정비 (2026-07-26)
- [C] **CF-1~CF-6** — 연금 수령액 자산 등록값 사용 · IRP 퇴직시점 잔액 반영 · 건보·세금 연도별 산정 · 금융소득 한도 1인별 · 세금 연간 조정열 · 검증

### 모바일 자산 타일 1줄 1개 (2026-08-14)
- [C] **TILE-1~TILE-6** — 주식/매각/부동산/연금/실물·기타 타일 전폭 가로 재배치 · 검증+렌더링 확인

### 주식 계좌 통합 관리 모드 (2026-08-23)
- [C] **ACC-1~ACC-6** — isAccountLevel 데이터 모델 · 생성폼 모드 선택 · 통합 타일/상세/배당 단일입력 · 검증+E2E

### UI 간소화 3단계 (2026-08-29)
- [C] **S1** — 간소화 1단계: 타일 정보 누락 보완 (c533e51)
- [C] **S2** — 간소화 2단계: 폼 필수 항목 축소 + 상세 옵션 접기 (4c230d3)
- [C] **S3** — 간소화 3단계: 하단 탭 4개(홈/자산/분석/설정) + 자산·분석 통합 페이지 + 구 라우트 리다이렉트 호환 (548bc08)
- [C] **S4** — 종목 등록 간소화 + 이력 연속 보기 + 차트 순서 통일 (9842322)

### 종목 상세 개선 (2026-08-29)
- [C] **D1** — 주식 상세 1줄 요약 바: 계좌·취득일·평단(인라인 편집)→현재가(등락%)·티커·갱신일 + 현재 가격 KPI 카드
- [C] **D2** — 최근 갱신일 표시 — 이력 마지막 기록일 (f424d53)
- [C] **D3** — 배당 탭 상단 요약 KPI 3장: 연간 예상·배당률·최근 1년 수령 (2100237)

### 시세 이력 백필 (2026-08-30)
- [C] **BF-1** — 시세 갱신 시 31일 이하 갭 자동 백필(직전값) + 테스트 2건 (7a8a9b6)
- [C] **BF-2** — 시세 업데이트 피드백: 모바일 메시지 표시(hidden sm:inline 제거) + 총액 변동 (6869058)
